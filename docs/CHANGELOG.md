...
## 2026-08-12 (bug fix) — Parent IC Lookup silent-render bug
- **Bug**: `/parent` (IC lookup) submitted successfully, the RPC completed and returned valid data, but the result silently never appeared — no data, no empty state, no error. Release-mode Flutter Web swallows the actual cause (a layout assertion failure), so there was nothing in the browser console or network tab pointing at it.
- **Root cause**: `ParentPortalBody` (shared between `/parent/:token` and `/parent`) had `ListView` as its root widget. That's correct when it's the sole child of a `Scaffold.body` (the original token route), but the new IC-lookup screen nests it inside its own page-level `ListView` (to host the search form above the results). A `ListView` embedded inside another `ListView`'s item throws `"Vertical viewport was given unbounded height"` — the assertion fails the whole subtree's layout pass silently in release mode.
- **Fix**: Changed `ParentPortalBody`'s root from `ListView` to a non-scrolling `Column` (`crossAxisAlignment: stretch`). `ParentPortalScreen` (`/parent/:token`) now wraps it in its own `SingleChildScrollView`; the IC-lookup screen's existing outer `ListView` already provides scrolling, so no wrapper needed there.
- **Diagnosis note for future AI sessions**: when a Flutter Web release build shows a state change (e.g. a loading spinner) followed by nothing — no error card, no console error — suspect a layout/render exception in a shared widget being reused in a new scroll context, not a data/RPC problem. Confirm the RPC layer is fine first (`curl` the RPC directly with the anon key from `app/env/local.json`), then reproduce with `flutter build web --debug` + a local static server, which surfaces the real `RenderBox`/`Viewport` assertion in the console instead of swallowing it.

## 2026-08-12 (later still) — Custom domain: d2csummit.online
- Moved hosting from `https://cgfusion.github.io/summit/` to the custom domain `https://d2csummit.online/`. `deploy-web.yml` now builds with `--base-href /` (was `/summit/`) and passes `cname: d2csummit.online` to the `peaceiris/actions-gh-pages` deploy step, so the CNAME file is (re)written into the `gh-pages` branch on every deploy instead of being wiped by the force-push.
