...
**Whole-school coverage (2026-09-15)**: originally only 252 students across 9 "D2C" classes (Tingkatan 1-2) had a card; extended to all 21 classes / Tingkatan 1-5 (609 students with a token) in two passes, both needed:

1. `supabase/seed/seed_data_2026-09-15.sql` (`scripts/generate_seed_sql_2026-09-15.py`): found the original 252 tokens had **224 linked to the wrong student** — a past import matched two spreadsheets by row position rather than identity. Used the class-sheet QR-recovery scan output (`results_named.csv`) as the fix source, matched by name.
2. **`results_named.csv` turned out to be wrong too** (`supabase/seed/fix_qr_reading_order_2026-09-15.sql`, `scripts/fix_qr_reading_order_2026-09-15.py`) — caught only because Raizal physically scanned a real card (ADDY AERAYYAN ARASSH BIN ANNUAR's) and it resolved to a different student (JANE SHERLYN JOHNSON). Root cause, confirmed by direct pixel inspection of the source PNGs: the tool that built `results_named.csv` decoded each class page's QR codes with a library (pyzbar) that returns detections in whatever order its algorithm finds them internally — not top-to-bottom reading order — then zipped that raw order directly against the alphabetically-sorted name list. This reversed the QR-to-name pairing on **every page** (**540 of the 609** tokens from pass 1 were wrong as a result). Fixed by decoding all 83 source PNGs directly, clustering detections into rows by y-coordinate, sorting each row left-to-right, and pairing that true reading order against the same name list.

Both passes used the same identity-preserving pattern: **re-link existing token rows to their correct owner** rather than reissuing new codes — a plain revoke-then-insert fails on the `token` unique constraint when swapping tokens that already exist in the table (a revoked row still holds its token text), so both scripts use a two-phase revoke-then-`upsert-on-conflict(token)` approach; see either script's header comment. Verified live via `fn_student_portal_data_by_qr` after the *second* pass, this time against the literal QR value decoded from the real card in question (not just re-querying what the database itself now says) — the previously-mislinked token now correctly resolves to ADDY AERAYYAN ARASSH BIN ANNUAR, plus 5 further random cross-class spot checks all correct. **Lesson recorded in `KNOWN_ISSUES.md`**: verifying a data-matching fix against the same database you just wrote to only proves internal consistency, not correctness against the real world — a claim of "verified" for identity-matching data should mean checked against an external, independent source (a real physical card, in this case), not a re-query.
