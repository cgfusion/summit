...
**Whole-school coverage (2026-09-15)**: originally only 252 students across 9 "D2C" classes (Tingkatan 1-2) had a card; `supabase/seed/seed_data_2026-09-15.sql` (generated by `scripts/generate_seed_sql_2026-09-15.py`) extended this to all 21 classes / Tingkatan 1-5 (609 students with a token). While doing this, found that **224 of the original 252 tokens were linked to the wrong student** — the original import matched two spreadsheets by row position rather than by identity, so a real, never-reissued physical card's code often ended up recorded against a different student than the one actually holding it. The fix used the actual class-sheet QR-recovery scan output (`results_named.csv`, grouped by class PDF + row number + name + code — verified to have zero duplicate names or codes) as the authoritative source, matched by name, and **re-linked existing token rows to their correct owner** rather than reissuing new codes (see the script's own header comment for the two-phase revoke-then-upsert-on-`token` approach this required, to avoid the `token` unique constraint colliding on itself mid-swap). Verified live post-migration via `fn_student_portal_data_by_qr`: the previously-mislinked token for AHMAD FARISH DANIAL BIN MORRIS now correctly resolves to HAZEL MERYCHELL EBLE, and his own corrected token resolves to him.
