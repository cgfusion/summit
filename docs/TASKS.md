...
| T-051 | Saringan Minda Sihat tab (Student Portal, 6th) — consent gate (instructions + "SAYA FAHAM" checkbox) in front of the external KPM screening link (`sepkm.com/msihatmenengah`), opened via `url_launcher`; no server-side data model, purely a client-side UI gate. Also switched the Student Portal's `TabBar` from scrollable to fixed with 2-line-wrapping labels so all 6 tabs fit without scrolling. | P1 | none | S |
| T-052 | Whole-school QR card rollout (609 students, 21 classes, up from 252/9). Took two correction passes: fixed 224 mis-linked tokens using `results_named.csv` as a name-matched source, then discovered (via Raizal physically scanning a real card) that *that* source was itself wrong on 540/609 rows due to a QR-decode reading-order bug — corrected by decoding the source PNGs directly and reading true page position. See `DATABASE.md`'s `qr_tokens` entry, `KNOWN_ISSUES.md` KI-019, `scripts/generate_seed_sql_2026-09-15.py` and `scripts/fix_qr_reading_order_2026-09-15.py`. | P1 | none | M |
