...
| T-052 | Whole-school QR card rollout (609 students, 21 classes, up from 252/9) + fixed 224 mis-linked tokens from the original row-order-based import bug, using the actual class-sheet QR-recovery scan (`results_named.csv`) as the authoritative name-matched source instead of the unreliable XEA4402 file's own QR Token column. See `DATABASE.md`'s `qr_tokens` entry and `scripts/generate_seed_sql_2026-09-15.py`. | P1 | none | M |
