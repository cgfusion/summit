...
| **Parent Portal** | `features/parent_portal` | Unauthenticated, token-gated OR IC-lookup view of student status for parents |
| **Student Portal & Suara Murid** | `features/student_portal` | Student QR Name Tag login, personal attendance/merit progress, live staff announcements, Suara Murid feedback/bullying reports box, an audience-targeted Sudut Info feed ("Inspirasi" tab), and the SAFE anti-bullying questionnaire — see `STUDENT_PORTAL_AND_VOICE.md`. `student_voice_submissions`'s former anon-read exposure (KI-014) was fixed 2026-09-14. |
| **Public Web Landing Page** | `features/landing` | Photo-background "Cyber" redesign of the D2C initiative page for whole school T1–T5, with a live Sudut Info carousel card and an AI Assistant FAB — see `LANDING_PAGE.md` |
