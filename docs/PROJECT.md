...
| **Discipline & Counseling** | `features/discipline_counseling` | SSDOP case tracking, UBK counseling sessions, Peti Suara Murid inbox, Special Announcements, Sudut Info (scheduled posts w/ image upload, audience-targeted), and staff-facing SAFE questionnaire results — 6 tabs, shipped in full; RBAC roles (`disiplin`/`kaunselor`) are a UI convention, **not enforced** at the DB level — see `DISCIPLINE_AND_COUNSELING.md` and `KNOWN_ISSUES.md` KI-015 |
| **Parent Portal** | `features/parent_portal` | Unauthenticated, token-gated OR IC-lookup view of student status for parents, plus an audience-targeted Sudut Info feed shown both on the IC-lookup login screen and the post-lookup result view (2026-09-14) |
| **Student Portal & Suara Murid** | `features/student_portal` | Student QR Name Tag login, personal attendance/merit progress, live staff announcements, Suara Murid feedback/bullying reports box, an audience-targeted Sudut Info feed shown both on the pre-login QR screen and the "Inspirasi" tab (2026-09-14), the SAFE anti-bullying questionnaire, and a Saringan Minda Sihat tab gating the external KPM mental-health screening link (T-051, 2026-09-14) — 6 tabs total, non-scrollable with 2-line-wrapping labels — see `STUDENT_PORTAL_AND_VOICE.md`. `student_voice_submissions`'s former anon-read exposure (KI-014) was fixed 2026-09-14. |
| **Public Web Landing Page** | `features/landing` | Photo-background "Cyber" redesign of the D2C initiative page for whole school T1–T5, with an AI Assistant FAB. **No longer has a Sudut Info card** — removed 2026-09-14, moved to the Student/Parent Portals — see `LANDING_PAGE.md` |
| **D2C AI Assistant** | `core/widgets/d2c_ai_assistant_dialog.dart` + `core/services/d2c_ai_assistant_service.dart` | Chat-style Q&A about the D2C program, opened from the Landing Page FAB. Despite "GEMINI INTELLIGENCE" branding, it is **100% local rule-based** in production — no API key is ever passed. See `COMPONENTS.md`/`KNOWN_ISSUES.md` KI-016 |
...
- **HTTP**: `http: ^1.2.0` — the one raw-HTTP dependency in the app, used exclusively by `D2CAiAssistantService`'s (currently-dormant) Gemini API call; every other network call goes through `supabase_flutter`.
- **File picking**: `file_picker: ^12.0.0` — added for Sudut Info's image upload flow (`discipline_counseling_screen.dart`).
...
- **Storage**: one Supabase Storage bucket, `sudut-info-banners` (public, added `20260818000001`) — see `DATABASE.md` Storage Buckets. Everything else remains bundled static assets.
...
8. **Every RPC that's reachable without a staff session is granted to `anon` explicitly and individually** — the default posture for new tables/functions is "staff-only via RLS", and public/anon access is an opt-in exception, not a default. **The "scoped to exactly one entity, never returns a list" half of this principle held through `fn_parent_portal_data`/`fn_student_discipline_summary`, but has since eroded**: `fn_parent_portal_data_by_ic` returns an array (by design, for multi-sibling guardians), `fn_active_sudut_info_posts` returns *every currently-active post school-wide* (a genuinely public broadcast list, reasonable for its purpose), and `fn_student_portal_data_by_qr` embeds array fields (`recent_attendance`, `submissions`, `announcements`) within its otherwise one-student-scoped response. None of these are necessarily wrong for what they do, but don't cite "never returns a list" as a still-true invariant when reviewing a new anon-reachable function — check each one's actual scoping on its own merits instead.
9. **Discipline & Counseling, Announcements, and Sudut Info writes are staff-wide (`to authenticated using (true)`), not role-gated**, even though `DISCIPLINE_AND_COUNSELING.md` documents a `disiplin`/`kaunselor`/`admin` RBAC model. `profiles.role` only has three values (`admin`/`teacher`/`staff`) — there is no schema support for the documented roles today. This was not a reasoned trade-off the way the Parent Portal's token model was; it's simply how the tables were built. Treat it as a known gap if a scoping conversation about tightening this ever comes up (`KNOWN_ISSUES.md` KI-015), not as an intentional design decision to defend.
10. **`student_voice_submissions` grants `anon` full `select`** (`using (true)`, no filter) — this is **not** in the same category as the above; it directly contradicts the "Sulit/Rahsia" confidentiality the Student Portal promises students for bullying/safety reports, and is documented as a bug, not a trade-off, in `KNOWN_ISSUES.md` KI-014. If you're reading this file to understand "how does this codebase think about anon access," do not use this table as a model to copy from.
...
- ~~Disiplin & Kaunseling (SSDOP/UBK) module~~ — **this shipped on 2026-08-10 (`TASKS.md` T-039) and is live in production.** A previous version of this section (and of `TASKS.md`'s Blocked table, and of `DISCIPLINE_AND_COUNSELING.md`'s status banner) incorrectly described it as deferred/not-started — that was a documentation error from two sessions editing this repo concurrently without seeing each other's commits, corrected 2026-09-13. What genuinely *is* still open for this module: the `disiplin`/`kaunselor` RBAC it describes is a UI convention only, not enforced at the DB level (see design decision #9 below, `KNOWN_ISSUES.md` KI-015).
