...
| **Discipline & Counseling** | `features/discipline_counseling` | SSDOP case tracking, UBK counseling sessions, Peti Suara Murid inbox, Special Announcements, and Sudut Info (scheduled posts w/ image upload) — 5 tabs, shipped in full; RBAC roles (`disiplin`/`kaunselor`) are a UI convention, **not enforced** at the DB level — see `DISCIPLINE_AND_COUNSELING.md` and `KNOWN_ISSUES.md` KI-015 |
| **Parent Portal** | `features/parent_portal` | Unauthenticated, token-gated OR IC-lookup view of student status for parents |
| **Student Portal & Suara Murid** | `features/student_portal` | Student QR Name Tag login, personal attendance/merit progress, live staff announcements, and Suara Murid feedback/bullying reports box — see `STUDENT_PORTAL_AND_VOICE.md`. **`student_voice_submissions` is publicly readable via the anon key — see `KNOWN_ISSUES.md` KI-014, the highest-priority open issue in this codebase.** |
