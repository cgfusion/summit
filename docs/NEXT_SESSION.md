...
**2026-09-15 — T-052, whole-school QR rollout + 224-token mismatch fix.** Raizal provided a fresh student master export (`XEA4402 Keseluruhan Murid as of 2026-09-15.xlsx`) and asked to migrate it, "especially the QR Token the last column." Diffing that column against the live `qr_tokens` table found a 91% mismatch rate (only 20/252 matched) — investigation showed most of the "different" tokens were the *same* known values, just linked to a *different* student (clean pairwise/cyclic swaps), the signature of a row-order bug, not a reissue. **Do not trust the XEA4402 file's own "QR Token" column** — it turned out to independently disagree with the real class-sheet scan output (`D:\Summit\System\qr_recovery\NewQR\qr_output\results_named.csv`) on about half its rows too. Raizal pointed to that recovery file as the real source; matched by name (601 exact + 7 verified spelling-variant fixups + 1 new student), it produced a clean, collision-free 609-student mapping. **The migration is applied** (`supabase/seed/seed_data_2026-09-15.sql`, generated by `scripts/generate_seed_sql_2026-09-15.py`) — QR coverage now spans all 21 classes / Tingkatan 1-5 (609 tokens, up from 252/9 classes), and 224 mis-linked tokens now point at their true owner. **Note for future re-runs of this class of migration**: a plain revoke-then-insert fails on the `token` unique constraint when tokens are being swapped between students (a revoked row still holds its old token text) — the two-phase approach (revoke each affected student's current row first, then `insert ... on conflict (token) do update` for the correct pairing) is required; see the script's header comment. Verified live via the actual `fn_student_portal_data_by_qr` RPC, not just SQL. Full writeup: `DATABASE.md`'s `qr_tokens` entry, `CHANGELOG.md` 2026-09-15, `TASKS.md` T-052.
